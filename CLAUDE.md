...
### 貸款輸入方式（成數 / 實際金額 雙模式）
「自訂多組貸款＿＿」欄位支援兩種輸入模式，用 radio 切換（`loanInputMode`／`loanInputMode_A`／`loanInputMode_B`）：
- `percent`：輸入貸款成數(%)，`loanAmount = totalPrice * (輸入值/100)`
- `amount`：輸入實際貸款金額(萬元)，`loanAmount = 輸入值*10000`，超過房屋總價時以總價封頂（不允許貸款超過房屋總價）
兩種模式都會反推出 `loanPct`（顯示用百分比）供表格/CSV 統一顯示「貸款金額：X萬 (Y%)」。切換模式時 `onLoanModeChange()` 會依目前房屋總價自動換算輸入框裡的數字（成數→金額或金額→成數），避免使用者切換後數字被誤讀成錯誤單位。
